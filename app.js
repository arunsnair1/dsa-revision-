...
const App = {
  ...
  renderDashboard() {
    ...
    const budgetContext = studyBudget.remainingMinutes < studyBudget.dailyBudget
      ? `Timer-aware budget: ${studyBudget.remainingMinutes} min left of ${studyBudget.dailyBudget} min daily budget`
      : `Daily budget: ${studyBudget.dailyBudget} min`;
  ...
  getTodayReviewTimeSpent() {
    const today = SpacedRepetitionEngine.getToday();
    return this.state.history
      .filter(h => h.date === today && h.feedback !== 'completed')
      .reduce((total, h) => total + (h.timeEstimate || TIME_RE_SOLVE), 0);
  },
  ...
  getRemainingStudyBudget() {
    ...
    const historyRemainingMinutes = Math.max(0, dailyBudget - this.getTodayReviewTimeSpent());
    ...
    return {
      ...
      remainingMinutes: Math.min(timerRemainingMinutes, historyRemainingMinutes)
    };
  },
